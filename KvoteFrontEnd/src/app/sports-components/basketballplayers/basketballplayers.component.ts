...
export class BasketballplayersComponent {
  ...
  ngOnInit() {
    ...
    this.sharedDataService.toggleLeagueDisplay("BasketballPlayers");
  ...
  getFlattenedAndSortedMatches() {
    ...
    const sport : string = "BasketballPlayers";
  ...
  addOddsToTicket(match : any, bettingGame: string, event: Event): void {
    if (this.isMobile && this.displaySidebar) {
      this.hideSideBar();
      return;
    }
